...
# --- 3. FUNCIÓN DE SEGURIDAD (ANTI-FLASH) ---
def safe_selectbox(label, options, key, **kwargs):
    """
    Componente blindado: Verifica el estado antes de renderizar.
    Si el valor en memoria no coincide con las opciones actuales, lo corrige silenciosamente.
    """
    # 1. Si la clave no existe, inicializar con el primero
    if key not in st.session_state:
        st.session_state[key] = options[0]
    
    # 2. Si la clave existe pero el valor NO está en las opciones (Causa del Flash Rojo)
    elif st.session_state[key] not in options:
        st.session_state[key] = options[0]
        
    # 3. Renderizar widget seguro
    return st.selectbox(label, options, key=key, **kwargs)

# Inicialización básica de variables que no son widgets selectbox
...
def perform_smart_update():
    ...
    # Mapeo de sugerencias a listas reales
    mappings = [
    ...
    for key, options in mappings:
        ...
        # Buscar coincidencia parcial segura
        for opt in options:
...
def perform_reset():
    ...
    # Reset de widgets a sus valores por defecto (index 0)
    reset_keys = ['char_select', 'shot_select', 'angle_select', 'lens_select', 'lit_select', 'sty_select', 'env_select', 'ar_select', 'ward_select', 'phy_select']
    for k in reset_keys:
        if k in st.session_state: del st.session_state[k] # Borrar para que safe_selectbox reinicie
...
# --- 8. MAIN ---
st.title("🎬 Grok Production Studio (V75)")
...
with st.form("main_form"):
    ...
    with t1:
        c1, c2 = st.columns(2)
        with c1:
            # Construcción dinámica de opciones
            ...
            # USO DE SAFE SELECTBOX (Adiós flash rojo)
        ...
        with col_tmpl:
            tpl = safe_selectbox("Plantilla Rápida", ["Seleccionar..."] + list(NARRATIVE_TEMPLATES.keys()), key="tpl_select")
        with col_btn:
    ...
    with t3:
        c1, c2 = st.columns(2)
        with c1: phy_med = safe_selectbox("Medio Físico", list(PHYSICS_LOGIC.keys()), key="phy_select")
