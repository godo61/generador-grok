...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        if 'last_img_name' not in st.session_state or st.session_state.last_img_name != uploaded_file.name:
            detect_and_set_ar(uploaded_file)
...
with t5:
    ...
    if dialogue_enabled:
        with st.container(border=True):
            ...
            d_txt = st.text_area("Guion:", placeholder="Texto...")
...
if st.button("✨ GENERAR PROMPT PRO", type="primary"):
    ...
    if shot or angle or lens:
        phrase = "Shot"
        if lens: phrase += f" on {lens}"
        if shot: phrase += f" using a {shot}"
        if angle: phrase += f" from a {angle} perspective"
        cine_parts.append(phrase)
