...
# --- GESTIÓN DE DEPENDENCIAS (A PRUEBA DE FALLOS) ---
# Intentamos importar el traductor. Si falla, usamos una función "tonta" que no traduce pero no rompe la app.
try:
    from deep_translator import GoogleTranslator
    TRANSLATOR_AVAILABLE = True
except ImportError:
    TRANSLATOR_AVAILABLE = False
    st.warning("⚠️ Librería 'deep_translator' no detectada. Los textos se usarán en español.")
...
def translate_to_english(text):
    ...
    # Intentamos traducir solo si la librería cargó
    if TRANSLATOR_AVAILABLE:
        try:
            return GoogleTranslator(source='auto', target='en').translate(text)
        except Exception:
            # Si falla la conexión, devolvemos el original
            return text
    else:
        return text
...
DEMO_SFX_COMMON = [
    "None (Ninguno)", "Footsteps on concrete (Pasos)", "Rain drops hitting surfaces (Gotas lluvia)",
    "Wind howling (Viento aullando)", "Explosion and debris (Explosión)",
    "Gunfire / Blaster shots (Disparos)", "Swords clashing (Espadas)",
    "Crowd cheering (Multitud vitoreando)", "Camera shutter clicks (Obturador)",
    "Keyboard typing (Tecleo)", "Siren wailing (Sirenas)", "Heartbeat (Latido)",
...
class GrokVideoPromptBuilder:
    ...
    # ESTA ES LA CLAVE: Manejar cualquier tipo de dato sin romperse
    def set_field(self, key, value):
        if value is None:
            ...
            self.parts[key] = str(value).strip()
...
# --- ZONA DE RESULTADOS (SIEMPRE VISIBLE) ---
