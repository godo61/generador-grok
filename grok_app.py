...
class GrokVideoPromptBuilder:
    ...
    def build(self) -> str:
        ...
        if self.is_img2video:
            ...
            # Instrucción de Audio (Lip Sync)
            if self.audio_filename:
                prompt.append(f"AUDIO SOURCE: '{self.audio_filename}' (Upload this file to the Video AI).")
                prompt.append("STRICT LIP-SYNC: Character's mouth must move in synchronization with the provided vocal track.")
        ...
        if p.get('dialogue_enabled'):
            ...
            # Si hay audio, añadimos contexto de performance
            if self.audio_filename:
...
if st.session_state.generated_output:
    ...
    st.code(st.session_state.generated_output, language="text")import streamlit as st
import re
